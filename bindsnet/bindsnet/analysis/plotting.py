...
def plot_input(
    ...
    if axes is None:
        ...
        ims = (
            axes[0].imshow(local_image, cmap="gray"),
            axes[1].imshow(local_inpy, cmap="gray"),
        )
...
def plot_spikes(
    ...
    if ims is None:
        ...
        ims = []
        for i, data in enumerate(spikes.items()):
            spikes = (
                data[1][
                    time[0] : time[1], n_neurons[data[0]][0] : n_neurons[data[0]][1]
                ]
            ...
            axes[i].set_xlim(time[0]-1, time[1]+1)
            axes[i].set_ylim(n_neurons[data[0]][0]-1, n_neurons[data[0]][1]+1)
        ...
        plt.setp(
            axes, xticks=[time[0], time[1]], yticks=[], 
                    xlabel="Simulation time", ylabel="Neuron index"
        )
        plt.tight_layout()
    else:
        for i, data in enumerate(spikes.items()):
            ...
            spike_times = np.array(spikes.nonzero()).T

            ims[i].set_offsets(spike_times)
...
def plot_weights(
    ...
    if not im:
        ...
        #shifted_cm = shiftedColorMap(cmap, start=wmin, midpoint=0., stop=wmax, name='shiftedcmap')
        #mid = -wmin/(wmax-wmin)
        #shifted_cm = shiftedColorMap(cmap, midpoint=mid, name='shiftedcmap')

        wmaxmax = max(-wmin, wmax)
        wmin1 = -wmaxmax
        wmax1 = wmaxmax

        im = ax.imshow(local_weights, cmap=shifted_cm, vmin=wmin1, vmax=wmax1, alpha=alpha)
        ...
        cax.set_ylim(wmin,wmax)
        c=plt.colorbar(im, cax=cax)
        #c.set_clim(vmin=wmin,vmax=wmax)
        ...
        ax.imshow(local_weights, cmap=shifted_cm, vmin=wmin1, vmax=wmax1, alpha=alpha)
...
def plot_assignments(
    ...
    # color = mcolors.ListedColormap(colors)

    color = plt.get_cmap("tab20", 20)

    locals_assignments = assignments.detach().clone().cpu().numpy()
    if props is not None:
        local_props = props.detach().clone().cpu().numpy()
    else:
        local_props = np.zeros(assignments.shape)+0.001
    ...
    if not im:
        ...
        if classes is None:
            ...
            #im = ax.matshow(locals_assignments, cmap=color, vmin=-1.5, vmax=9.5, alpha = 0.5)
            im = ax.matshow(locals_assignments+(1-local_props), cmap=color, vmin=0., vmax=10.)
        else:
            #color = plt.get_cmap("RdBu", len(classes) + 1)
            im = ax.matshow(
                locals_assignments+(1-local_props), cmap=color, vmin=0., vmax=len(classes),
            )
        ...
        #if props is not None:
        #    im.set_data(local_props, cmap=plt.get_cmap('gray'))
    ...
    if not ims:
        ...
        if n_subplots == 1:  # Plotting only one image
            ...
            for i, v in enumerate(voltages.items()):
                ...
                args = (v[0], time[0], time[1], n_neurons[v[0]][0], n_neurons[v[0]][1])
