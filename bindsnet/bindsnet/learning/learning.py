...
class LearningRule(ABC):
    ...
    def __init__(
        ...
        dead_cell_rate: Optional[float] = None,
        use_adam: bool = False,
        **kwargs
    ) -> None:
        # language=rst
        ...
        self.weight_decay = weight_decay if weight_decay else 0.0
        
        if dead_cell_rate:
            print("using dead cell mech: ", dead_cell_rate)
        self.dead_cell_rate = dead_cell_rate


    def adaptive(self, new_update):

        self.connection.g2 = self.connection.g2*(1-1/self.rms_tau) + new_update**2/self.rms_tau

        return new_update/(torch.sqrt(self.connection.g2) + 1e-4)
    ...
    def update(self) -> None:
      ...
      with torch.no_grad():
        ...
        if self.weight_decay and not self.use_adam:
            self.connection.w.data -= self.nu0 * self.weight_decay * self.connection.w

        if (self.dead_cell_rate is not None):
            self.connection.w.data += 1e-2 * (self.target.rate < self.dead_cell_rate) * self.connection.w / self.connection.w_abs_sum


        # Bound weights.
        if ((self.connection.wmin != -np.inf and self.connection.wmin != None) or 
          (self.connection.wmax != np.inf and self.connection.wmax != None)) and \
          not isinstance(self, NoOp):
            self.connection.w.data.clamp_(self.connection.wmin, self.connection.wmax)
...
class Triplet(LearningRule):
    ...
    def _connection_update(self, **kwargs) -> None:
        ...
        if self.use_adam and not hasattr(self, 'optimizer'):
          print(f"Using Adam. lr: {self.nu0}, beta: {self.beta}, beta2: {self.beta2}")
          # self.tw = torch.tensor(self.connection.w, requires_grad=True)
          self.optimizer = optim.Adam([self.connection.w], lr=self.nu0, betas=(self.beta, self.beta2), weight_decay=self.weight_decay)
        
        batch_size = self.source.batch_size

        source_s = self.source.s.view(batch_size, -1).unsqueeze(2).float()
        source_x = self.source.x.view(self.source.traces, batch_size, -1).unsqueeze(3)
        source_rate = self.source.rate.view(-1).unsqueeze(0).unsqueeze(2)
        
        target_s = self.target.s.view(batch_size, -1).unsqueeze(1).float()
        target_x = self.target.x.view(self.target.traces, batch_size, -1).unsqueeze(2)
        target_rate = self.target.rate.unsqueeze(0).unsqueeze(1)
        ...
        if self.nu[1]:
            if self.bcm_rule:
              update -= self.nu[1] * self.reduction(torch.bmm(source_s-self.connection.subtract_input_mean*source_rate, target_x[0]*target_rate**2), dim=0)
            else: # - w*y2, Oja-like LTD
              update -= self.nu[1] * torch.mul(self.connection.w.data, target_rate.squeeze(0)**2)

        # Post-synaptic update.
        if self.nu[0]:
            update += self.nu[0] * self.reduction(torch.bmm(source_x[0]-self.connection.subtract_input_mean*source_rate, target_x[1]*target_s), dim=0)


        if self.use_adam:
          self.connection.w.grad = torch.tensor(-update)
          self.optimizer.step()
        else:
          if self.connection.attenuation_factor:
            ada_update = self.adaptive(update)
            wsize = self.connection.source.n
            self.connection.w[:wsize//2] += self.nu0 * ada_update[:wsize//2] / self.connection.attenuation_factor[0]
            self.connection.w[wsize//2:] += self.nu0 * ada_update[:wsize//2] / self.connection.attenuation_factor[1]
          else:
            ada_update = self.adaptive(update)
            self.connection.w += self.nu0 * ada_update
          
        super().update()


class AntiHebb(LearningRule):
    ...
    def _connection_update(self, **kwargs) -> None:
        ...
        # Pre-synaptic update.
        if self.nu[1]:
            update -= self.nu[1] * self.reduction(torch.bmm(source_s, target_x[0] - self.theta), dim=0) # vogels+w.d.

        # Post-synaptic update.
        if self.nu[0]:
            update -= self.nu[0] * self.reduction(torch.bmm(source_x[0], target_s - self.theta), dim=0) # vogels+w.d.
        ...
        if self.no_self_connections:
          self.connection.w[torch.eye(self.connection.w.size(0)).byte()] = 0
