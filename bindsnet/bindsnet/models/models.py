...
from ..learning import PostPre, Triplet, LearningRule, AntiHebb, wSTDP
from ..network import Network
from ..network.nodes import Input, LIFNodes, DiehlAndCookNodes, CurrentLIFNodes, AdaptiveLIFNodes
...
class EINetwork(Network):
    def __init__(
        ...
        reset: float = -65.,
        I_tau: float = 5.,
        rate_tau: float = 10000, 
        x_trace_tau: float = 40.,
        y_trace_tau_LTD: float = 40.,
        y_trace_tau_LTP: float = 40.,
        do_norm: int = 0,
        norm: float = 10., 
        norm_type: int = 2,
        rule: LearningRule = Triplet,
        weight_decay: float = 0.,
        dead_cell_rate: Optional[float] = None,
        i_weight_decay: float = 0.,
        noise: float = 0.,
        nu0: float = 1e-2,
        rec_nu0: float = 1e-2,
        theta: float = 0.,
        nu: Union[float, Sequence[float]] = (1, 1),
        wmin: Optional[float] = 0.,
        wmax: Optional[float] = None,
        reduction: Optional[callable] = torch.sum,
        inpt_shape: Optional[Iterable[int]] = None,
        subtract_input_mean: bool = False,
        attenuation_factor: Optional[Sequence[float]] = None,
        rep_init: Optional[Sequence[float]] = None,
        beta: Optional[float] = 0.9,
        beta2: Optional[float] = 0.9,
        use_adam: bool = False,
        bcm_rule: bool = True,
        **kwargs, # to ignore the two type extra args
    ) -> None:
        # language=rst
        """
        ...
        self.add_layer(Input(n=self.n_inpt, shape=self.inpt_shape, 
                             traces=1, tc_trace=[x_trace_tau], rate_tau=rate_tau), 
                        name="X")
        self.add_layer(
            CurrentLIFNodes(
                n=self.n_neurons,
                ...
                tc_decay=mem_tau,
                tc_i_decay=I_tau,
                tc_trace=[y_trace_tau_LTD, y_trace_tau_LTP],
            ),
            name="Y",
        )

        w = exc * torch.randn(self.n_inpt, self.n_neurons)
        w[:self.n_inpt//2] = rep_init[0]*w[:self.n_inpt//2]
        w[self.n_inpt//2:] = rep_init[1]*w[self.n_inpt//2:]
        ff_connection = Connection(
        ...
        recurrent_connection = Connection(
            ...
            target=self.layers["Y"],
            w=w,
            update_rule=AntiHebb,
            nu0=rec_nu0,
            reduction=reduction,
            wmin=None,
            wmax=0,
            weight_decay=i_weight_decay,
            theta=theta,
        )
        ...
        self.layers["Y"].rate[:] = 0.05
        self.layers["X"].rate[:] = 0.05
