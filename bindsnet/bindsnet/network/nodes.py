...
class Nodes(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        #for it in range(self.traces):
        if self.traces:
            self.register_buffer("x", torch.Tensor())  # Firing traces.
            self.register_buffer(
                "tc_trace", torch.tensor(tc_trace).view(-1)
            )  # Time constant of spike trace decay.
            if self.traces_additive:
                self.register_buffer(
                    "trace_scale", torch.tensor(trace_scale).view(-1)
                )  # Scaling factor for spike trace.
            self.register_buffer(
                "trace_decay", torch.empty_like(self.tc_trace).view(-1)
            )  # Set in compute_decays.


        if self.with_rate:
            self.register_buffer("rate", torch.Tensor())  # Firing traces.
            self.register_buffer(
                "rate_tau", torch.tensor(rate_tau).view(-1)
            )  # Time constant of spike trace decay.
            self.register_buffer(
                "rate_decay", torch.empty_like(self.rate_tau).view(-1)
            )  # Set in compute_decays.
    ...
    def forward(self, x: torch.Tensor) -> None:
        ...
        for it in range(self.traces):
            ...
            self.x[it] *= self.trace_decay[it]

            if self.traces_additive:
                #self.x[it] += self.trace_scale[it] * self.s.float()
                self.x[it] += self.s.float() / self.tc_trace[it]
            else:
                self.x[it].masked_fill_(self.s != 0, 1)

        if self.with_rate:
            self.rate *= self.rate_decay
            self.rate += self.s.float().mean(dim=0) / self.rate_tau
    ...
    def compute_decays(self, dt) -> None:
        ...
        if self.with_rate:
            self.rate_decay = torch.exp(
                -self.dt / self.rate_tau
            )  # Spike trace decay (per timestep).
    ...
    def set_batch_size(self, batch_size) -> None:
        ...
        if self.traces:
            self.x = torch.zeros(self.traces, batch_size, *self.shape, device=self.x.device)

        if self.with_rate:
            self.rate = torch.zeros(*self.shape, device=self.x.device)
...
class CurrentLIFNodes(Nodes):
    ...
    def __init__(
        ...
        self.register_buffer("noise", torch.tensor(noise))  # Post-spike reset voltage.
    ...
    def forward(self, x: torch.Tensor) -> None:
        ...
        # Integrate inputs.
        self.i += self.dt*x/self.tc_i_decay
        #if(x.sum() > 1):
        #    print('yes', x, self.dt, self.tc_i_decay)
        #self.i += x
        self.v += (self.refrac_count <= 0).float() * self.i

        # Noise
        self.v += self.noise*torch.randn(self.shape, device=self.v.device)
    ...
    def reset_state_variables(self) -> None:
        ...
        super().reset_state_variables()
        #self.v.fill_(self.rest)  # Neuron voltages.
        self.v.uniform_(self.rest,self.thresh)  # Neuron voltages.
...
#### Need to implement input types to make this work.
class ConducaanceLIFNodes(Nodes):
    # language=rst
    """
    Layer of conductance-based leaky integrate-and-fire (LIF) neurons
    ...
    def __init__(
        ...
        refrac: Union[int, torch.Tensor] = 5,
        tc_decay: Union[float, torch.Tensor] = 100.0,
        tc_g_decay: Union[float, torch.Tensor] = 2.0,
        lbound: float = None,
        # **kwargs,
    ) -> None:
        # language=rst
        ...
        self.register_buffer(
            "tc_g_decay", torch.tensor(tc_g_decay)
        )  # Time constant of synaptic input current decay.
        self.register_buffer(
            "g_decay", torch.empty_like(self.tc_i_decay)
        )  # Set in compute_decays.
    ...
    def forward(self, x: torch.Tensor) -> None:
        ...
        dv = self.g_gaba*(self.E_gaba-self.v) + self.g_ampa*(self.E_ampa-self.v) + self.g_l*(self.E_l-self.v)
        ...
        # Integrate inputs.
        self.g += self.dt*x/self.tc_g_decay
        #if(x.sum() > 1):
        #    print('yes', x, self.dt, self.tc_i_decay)
        #self.i += x
        self.v += (self.refrac_count <= 0).float() * self.i
    ...
    def compute_decays(self, dt) -> None:
        ...
        self.g_decay = torch.exp(
            -self.dt / self.tc_g_decay
        )  # Synaptic input current decay (per timestep).
