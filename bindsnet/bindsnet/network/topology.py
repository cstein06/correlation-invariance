...
class AbstractConnection(ABC, Module):
    ...
    def __init__(
        ...
        attenuation_factor: Sequence[float] = None,
        use_adam: bool = False,
        **kwargs
    ) -> None:
        # language=rst
        ...
        self.do_norm = kwargs.get("do_norm", None)
        self.norm_type = kwargs.get("norm_type", None)
...
class Connection(AbstractConnection):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        super().__init__(source, target, nu0, nu, reduction, weight_decay, **kwargs)

        w = kwargs.get("w", None)
        if w is None:
            if (self.wmin != -np.inf and self.wmin != None) or (self.wmax != np.inf and self.wmax != None):
        ...
        self.w = Parameter(w, requires_grad=self.use_adam)

        b = kwargs.get("b", None)
        if b is not None:
            self.b = Parameter(b, requires_grad=self.use_adam)
        else:
            self.b = None
        ...
        self.register_buffer(
                "w_abs_sum", torch.zeros([1,target.n])
            )  
        self.register_buffer(
                "g2", torch.ones(source.n, target.n)
            )  
        self.get_norm()
    ...
    def compute(self, s: torch.Tensor) -> torch.Tensor:
        ...
        if self.subtract_input_mean:
          effective_input = s.view(s.size(0), -1).float() - self.source.rate.view(-1)
        else:
          effective_input = s.view(s.size(0), -1).float()
          
        if self.attenuation_factor:
          
          half_n = effective_input.shape[1]//2
          effective_input[:,:half_n] *= self.attenuation_factor[0]
          effective_input[:,half_n:] *= self.attenuation_factor[1]
        
        
        with torch.no_grad():
          if self.b is None:
              post = effective_input @ self.w
          else:
              post = effective_input @ self.w + self.b
    ...
    def get_norm(self):
        ...
        self.w_abs_sum = torch.norm(self.w, dim=0, p=self.norm_type).unsqueeze(0)
        self.w_abs_sum[self.w_abs_sum == 0] = 1.0
        return self.w_abs_sum
    ...
    def normalize(self) -> None:
        ...
        ``self.norm``.
        """
        if self.do_norm is not None:
            self.w *= self.norm / self.get_norm()
