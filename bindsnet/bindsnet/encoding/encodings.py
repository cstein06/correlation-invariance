...
def single(
    ...
    **kwargs
    ...
    data = np.copy(data)
    quantile = np.quantile(data, 1 - sparsity)
    s = np.zeros([time, *shape], device=device)
    s[0] = np.where(data > quantile, np.ones(shape), np.zeros(shape))
    return torch.Tensor(s).byte()


def repeat(data: torch.Tensor, time: int, dt: float = 1.0, **kwargs) -> torch.Tensor:
    # language=rst
    """
    :param data: Repeats a tensor along a new dimension in the 0th position for
    ...
    """
    time = int(time / dt)
    return data.repeat([time, *([1] * len(data.shape))])
...
def poisson(data: torch.Tensor, time: int, dt: float = 1.0, device="cpu", **kwargs
) -> torch.Tensor:
    # language=rst
    """
    ...
    #rate[data != 0] = 1 / data[data != 0] * (1000 / dt)
    rate = 1 / (data+eps) * (1000 / dt)
...
def rank_order(
    data: torch.Tensor, time: int, dt: float = 1.0, **kwargs
) -> torch.Tensor:
    # language=rst
    """
